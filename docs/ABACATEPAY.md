...
- **Nome, descrição e imagem no checkout (billing v1):** O servidor envia **`name`** e **`description`** em cada item da cobrança (textos do plano **MAPH PRO 3D** no código). Para **logotipo**, defina **`ABACATEPAY_CHECKOUT_PRODUCT_IMAGE_URL`** (URL **https** pública, ex.: `https://seudominio.com/logo.png` após publicar um PNG em **`public/logo.png`**). Opcional: `_PRO` e `_LIFETIME` para imagens diferentes. Se a AbacatePay recusar o campo `imageUrl` na v1, remova a variável e use a imagem do produto no **painel** (fluxo com `prod_` + API v2).
